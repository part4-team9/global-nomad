...
export default function ActivityCard({ activity, onClick }: ActivityCardProps) {
  return (
    <div
      key={activity.id}
      className="relative my-2 size-[186px] flex-shrink-0 cursor-pointer rounded-[24px] transition-transform duration-300 hover:-translate-y-2 hover:drop-shadow-xl mobile:size-[384px]"
      onClick={onClick}
    >
      <Image src={activity.bannerImageUrl} alt={activity.title} fill priority className="rounded-[24px] object-cover" />
      <div className="absolute inset-x-0 bottom-0 h-3/5 rounded-[24px]" style={{ background: 'linear-gradient(to top, #1B1B1B, transparent)' }} />
